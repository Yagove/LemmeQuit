...
struct MainPatientView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ToastAIView(
                ...
                message: geminiVM.currentResponse,
                isLoading: geminiVM.isLoading
            )
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Button(action: requestAIAdvice) {
                    ...
                    .background(
                        Capsule()
                            .fill(geminiVM.isLoading ? Color.gray : Color.purple)
                    )
                }
                .disabled(geminiVM.isLoading)
            }
    ...
    private func requestAIAdvice() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            Task {
                await self.geminiVM.requestAdviceAsync(for: currentUser)
                await MainActor.run {
